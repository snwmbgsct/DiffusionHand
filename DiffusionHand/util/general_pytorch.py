...
def crop_image_from_xy_original(image, crop_location, crop_size, scale=1.0):
    """
...
def crop_image_from_xy(image, crop_location, crop_size, scale=1.0):
    ...
    assert len(image.size()) == 4, "Image needs to be of shape [batch, channels, height, width]"

    scale = scale.view(-1)
    crop_location = crop_location.float()
    crop_location = crop_location.view(batch_size, 2)
    crop_size = float(crop_size)

    crop_size_scaled = crop_size / scale
    y1 = (crop_location[:, 0] - crop_size_scaled // 2).long()
    x1 = (crop_location[:, 1] - crop_size_scaled // 2).long()

    crop_height, crop_width = int(crop_size), int(crop_size)
    image_crop = torch.zeros((batch_size, channels, crop_height, crop_width))

    for i in range(batch_size):
        image_crop[i] = F.crop(image[i], y1[i], x1[i], crop_height, crop_width)
...
class EvalUtil:
    ...
    def feed(self, keypoint_gt, keypoint_vis, keypoint_pred):
        ...
        # calc euclidean distance
        diff = keypoint_gt - keypoint_pred
        # if diff.device.type and diff.device.type == 'cuda':
        #     diff = diff.cpu()
        #     euclidean_dist = np.sqrt(np.sum(np.square(diff.numpy()), axis=1))
        # else:
        euclidean_dist = np.sqrt(np.sum(np.square(diff), axis=1)) #.cpu().numpy()
